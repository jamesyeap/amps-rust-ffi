...
- [x] **BUG-1: `copy_to_thread_local_buffer` clobbers previous values** — Fixed by using a `DEFINE_MESSAGE_ACCESSOR` macro that gives each accessor its own `static thread_local std::string buffer`, so calling multiple accessors no longer invalidates earlier pointers.
- [x] **BUG-2: `CallbackContext` memory leak (C++)** — Fixed by adding `CATCH_AMPS_EXCEPTIONS_WITH_CLEANUP` macro that `delete`s the `CallbackContext` in every catch branch. Used in `subscribe`, `sow`, and `sow_and_subscribe`.
- [x] **BUG-3: `disconnect_handler_trampoline` is a no-op (Rust)** — Fixed by changing the handler signature to `FnMut() + Send + 'static` (removing the unsound `&mut Client` parameter) and actually calling the handler in the trampoline.
- [x] **BUG-4: Disconnect handler `user_data` never freed (Rust)** — Fixed by storing the raw pointer in `Client._disconnect_handler` and freeing it on drop and when replaced by a new handler.
- [x] **BUG-5: `DisconnectContext` memory leak (C++)** — Fixed by adding `delete ctx` in all catch branches of `amps_ffi_client_set_disconnect_handler`.
- [x] **BUG-6: Unsound `unsafe impl Sync for Message`** — Fixed by removing `unsafe impl Sync for Message`. `Message` remains `Send` only.
