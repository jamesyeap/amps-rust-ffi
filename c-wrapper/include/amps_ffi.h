...
/* Opaque handle types — prefixed with amps_ffi_ to avoid collision with
 * the AMPS C client's own amps_client_t / amps_message_t typedefs. */
typedef struct amps_ffi_client_handle* amps_ffi_client_t;
typedef struct amps_ffi_message_handle* amps_ffi_message_t;
typedef struct amps_ffi_command_handle* amps_ffi_command_t;
...
typedef void (*amps_ffi_message_handler_t)(amps_ffi_message_t message, void* user_data);

/* Disconnect handler callback type */
typedef void (*amps_ffi_disconnect_handler_t)(amps_ffi_client_t client, void* user_data);
