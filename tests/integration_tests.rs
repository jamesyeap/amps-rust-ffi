...
macro_rules! skip_if_no_amps {
    () => {
        if !is_amps_available() {
            ...
            eprintln!("Start the server with: docker-compose -f tests/docker/docker-compose.yml up -d");
            return;
        }
    };
}
...
fn test_connect_and_publish() {
    ...
    client.connect(&amps_uri()).expect("Failed to connect to AMPS");
    ...
    let seq2 = client.publish_with_expiration("test-topic", r#"{"message": "with expiration"}"#, 60)
...
fn test_delta_publish() {
    ...
    client.publish("sow-test", r#"{"id": "1", "name": "test", "value": 100}"#)
        .expect("Failed to publish initial record");
    
    // Then publish a delta update
    client.delta_publish("sow-test", r#"{"id": "1", "value": 200}"#)
...
fn test_subscribe_and_receive() {
    ...
    publisher.connect(&amps_uri()).expect("Failed to connect publisher");
    publisher.logon(None, 5000).expect("Failed to logon publisher");
    
    // Create subscriber client
    let mut subscriber = Client::new("test-subscriber").expect("Failed to create subscriber");
    subscriber.connect(&amps_uri()).expect("Failed to connect subscriber");
    subscriber.logon(None, 5000).expect("Failed to logon subscriber");
    ...
    // Subscribe
    subscriber.subscribe(topic, None, move |msg| {
    ...
    // Publish a message
    publisher.publish(topic, test_message).expect("Failed to publish");
    ...
    assert!(!messages.is_empty(), "Expected at least one message to be received");
    
    // Parse the received message and verify content
    let received: serde_json::Value = serde_json::from_str(&messages[0])
        .expect("Failed to parse received message as JSON");
    assert_eq!(received["test"], "data");
    assert_eq!(received["value"], 42);
    
    // Clean up
    subscriber.unsubscribe_all().expect("Failed to unsubscribe");
    subscriber.disconnect().expect("Failed to disconnect subscriber");
    publisher.disconnect().expect("Failed to disconnect publisher");
}
...
fn test_subscribe_with_filter() {
    ...
    let mut subscriber = Client::new("test-filter-subscriber").expect("Failed to create subscriber");
    ...
    subscriber.subscribe(topic, Some("/value > 50"), move |msg| {
        let data = msg.data().to_string();
        received_clone.lock().unwrap().push(data);
    }).expect("Failed to subscribe");
    ...
    publisher.publish(topic, r#"{"id": "1", "value": 75}"#).expect("Failed to publish");
    publisher.publish(topic, r#"{"id": "2", "value": 100}"#).expect("Failed to publish");
    
    // Publish messages that should NOT match the filter
    publisher.publish(topic, r#"{"id": "3", "value": 25}"#).expect("Failed to publish");
    publisher.publish(topic, r#"{"id": "4", "value": 10}"#).expect("Failed to publish");
    ...
    for msg in messages.iter() {
        ...
        assert!(value > 50, "Received message with value {} that should have been filtered", value);
    }
...
fn test_sow_query() {
    ...
    publisher.publish(topic, r#"{"id": "1", "name": "alice", "age": 30}"#).expect("Failed to publish");
    publisher.publish(topic, r#"{"id": "2", "name": "bob", "age": 25}"#).expect("Failed to publish");
    publisher.publish(topic, r#"{"id": "3", "name": "charlie", "age": 35}"#).expect("Failed to publish");
    ...
    // Execute SOW query
    client.sow(topic, None, move |msg| {
        let data = msg.data().to_string();
    ...
    assert!(results.len() >= 3, "Expected at least 3 SOW records, got {}", results.len());
...
fn test_sow_query_with_filter() {
    ...
    // Create publisher
    let mut publisher = Client::new("test-sow-filter-publisher").expect("Failed to create publisher");
    publisher.connect(&amps_uri()).expect("Failed to connect");
    publisher.logon(None, 5000).expect("Failed to logon");
    ...
    publisher.publish(topic, &format!(r#"{{"id": "sow-filt-{}-1", "category": "A", "amount": 100}}"#, timestamp))
        .expect("Failed to publish");
    publisher.publish(topic, &format!(r#"{{"id": "sow-filt-{}-2", "category": "B", "amount": 200}}"#, timestamp))
        .expect("Failed to publish");
    publisher.publish(topic, &format!(r#"{{"id": "sow-filt-{}-3", "category": "A", "amount": 150}}"#, timestamp))
        .expect("Failed to publish");
    ...
    for result in results.iter() {
        ...
        if let Some(id) = parsed["id"].as_str() {
            if id.starts_with(&format!("sow-filt-{}-", timestamp)) {
                assert_eq!(parsed["category"], "A", "Received non-A category record that should have been filtered");
            }
        }
    }
...
fn test_sow_and_subscribe() {
    ...
    // Pre-populate SOW
    publisher.publish(topic, r#"{"id": "sowsub-1", "status": "initial"}"#).expect("Failed to publish");
    
    std::thread::sleep(Duration::from_millis(100));
    
    // Create subscriber
    let mut subscriber = Client::new("test-sowsub-subscriber").expect("Failed to create subscriber");
    subscriber.connect(&amps_uri()).expect("Failed to connect");
    subscriber.logon(None, 5000).expect("Failed to logon");
    ...
    subscriber.sow_and_subscribe(topic, None, move |msg| {
        let data = msg.data().to_string();
    ...
    // Publish an update
    publisher.publish(topic, r#"{"id": "sowsub-1", "status": "updated"}"#).expect("Failed to publish update");
    publisher.publish(topic, r#"{"id": "sowsub-2", "status": "new"}"#).expect("Failed to publish new");
    ...
    assert!(received.len() >= 3, "Expected at least 3 messages (SOW + 2 updates), got {}", received.len());
...
fn test_unsubscribe() {
    ...
    // Subscribe
    client.subscribe(topic, None, move |msg| {
    ...
    publisher.publish(topic, r#"{"test": "after-unsubscribe"}"#).expect("Failed to publish");
...
fn test_exception_handling_already_connected() {
    ...
    // First connection
    client.connect(&amps_uri()).expect("First connect should succeed");
...
fn test_exception_handling_not_connected() {
    ...
    assert!(result.is_err(), "Expected publish to fail when not connected");
}
...
fn test_set_heartbeat() {
    ...
    client.set_heartbeat(30, 60).expect("Failed to set heartbeat");
...
fn test_multiple_clients() {
    ...
    for (i, client) in clients[1..].iter_mut().enumerate() {
        ...
        client.subscribe(topic, None, move |_msg| {
            *count.lock().unwrap() += 1;
    ...
    clients[0].publish(topic, r#"{"test": "multi-client"}"#).expect("Failed to publish");
    
    std::thread::sleep(Duration::from_millis(500));
    
    // Verify messages were received
    let count = received_count.lock().unwrap();
    assert!(*count >= 4, "Expected at least 4 messages (one per subscriber), got {}", *count);
...
fn test_message_properties() {
    ...
    publisher.publish(topic, test_data).expect("Failed to publish");
    ...
    let (received_topic, received_data, received_command, data_len, has_data) = msg_info.clone().unwrap();
